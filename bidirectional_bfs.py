...
class BidirectionalBFSRobo:
    ...
    def bidirectionalBFS(self, start, goal):
        if start == goal:
            return 0
        ...
        while start_queue or goal_queue:
            if start_queue:
                ...
                current_dist = start_visited[current]
                
                for neighbor in self.getNeighbors(current):
                    if neighbor in goal_visited:
                        return current_dist + 1 + goal_visited[neighbor]
                        
                    if neighbor not in start_visited:
                        start_visited[neighbor] = current_dist + 1
                        start_queue.append(neighbor)
            
            if goal_queue:
                current = goal_queue.popleft()
                current_dist = goal_visited[current]
                
                for neighbor in self.getNeighbors(current):
                    if neighbor in start_visited:
                        return current_dist + 1 + start_visited[neighbor]
                        
                    if neighbor not in goal_visited:
                        goal_visited[neighbor] = current_dist + 1
                        goal_queue.append(neighbor)
        
        return -1

    def distMinAtoB(self, src, dst):
        self.dist = self.bidirectionalBFS(src, dst)
